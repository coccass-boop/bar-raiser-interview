...
st.markdown("""
    <style>
    /* 아이콘 버튼 전용 컨테이너: 텍스트와 수직 중앙 정렬 핵심 */
    .icon-box {
        display: flex;
        align-items: center;
...
def generate_questions_by_category(category, level, resume_file, jd_text):
    prompt = f"[Role] Bar Raiser. [Value] {BAR_RAISER_CRITERIA[category]}. [Task] 10 Questions JSON."
    try:
...
with st.sidebar:
    ...
    with tab2:
        jd_text = st.text_area("내용 붙여넣기", height=150)
        jd_final = jd_text if jd_text else jd_final
...
# Wide 모드 토글 (글자 깨짐을 유발하는 비율 조정 대신 컨테이너 전환 방식 사용)
